...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template match="dri:options">
        <div id="ds-options" class="word-break hidden-print">
            <xsl:if test="not(contains(/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='request'][@qualifier='URI'], 'discover'))">
                <div id="ds-search-option" class="ds-option-set visible-xs visible-sm hidden-md hidden-lg">
    ...
    <xsl:template match="dri:options/dri:list" priority="3">
        <xsl:choose>
            <xsl:when test="(./@n='account' and //dri:userMeta[@authenticated='no'])">
                <!-- do not account menu for normal user -->
            </xsl:when>
	    <xsl:when test="(contains(./@id, 'browse'))"> 
	    <!-- <xsl:when test="(./@n = 'global')"> -->
		    <!-- do show special browse menu -->
		    <div class="visible-xs visible-sm hidden-md hidden-lg">
			    <h2 class="ds-option-set-head"><i18n:text>xmlui.ArtifactBrowser.Navigation.head_browse</i18n:text></h2>
			<div id="aspect_viewArtifacts_Navigation_list_browse" class="list-group">
		    		<a class="list-group-item ds-option" href="/community-list"><i18n:text>xmlui.correspondences</i18n:text></a>
				    <a class="list-group-item ds-option" href="/search-filter?field=correspondent"><i18n:text>xmlui.correspondents</i18n:text></a> 
			</div>
		    </div>
            </xsl:when>
	    <xsl:otherwise>
	...
	<xsl:choose>
		<xsl:when test="contains(../@id, 'SidebarFacetsTransformer.list.lang')">
			<xsl:variable name="lang"><xsl:value-of select="substring-before(., ' (')"/></xsl:variable>
			<i18n:text><xsl:value-of select="$lang"/></i18n:text><xsl:value-of select="concat(' (', substring-after(., ' ('))"/>
		</xsl:when>
		<xsl:otherwise>
			<xsl:apply-templates />
		</xsl:otherwise>
    ...
    <xsl:template match="dri:options//dri:item[dri:xref]">
	    ...
	    <xsl:choose>
		<xsl:when test="contains(dri:xref/@target, 'filtertype=lang')">
 		<!-- translate language facettes -->
			<xsl:variable name="lang"><xsl:value-of select="substring-before(dri:xref, ' (')"/></xsl:variable>
			<xsl:variable name="number"><xsl:value-of select="substring-after(dri:xref, ' (')"/></xsl:variable>
			<i18n:text><xsl:value-of select="$lang"/></i18n:text><xsl:value-of select="concat(' (', $number)"/>
		</xsl:when>
