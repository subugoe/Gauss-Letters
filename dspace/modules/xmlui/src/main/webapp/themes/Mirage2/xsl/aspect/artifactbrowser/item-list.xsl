...
<xsl:stylesheet
	...
	<xsl:choose>
	    ...
	    <xsl:otherwise>
			...
			<xsl:choose>
			    <xsl:when test="count(./mets:fileSec/mets:fileGrp[@USE='ORIGINAL' or @USE='CONTENT']/mets:file) &gt; 0">
    ...
    <xsl:template match="dim:dim" mode="itemSummaryList-DIM-metadata">
		...
		<xsl:choose>
                        <xsl:when test="$text = 'yes'">
                                <img src="/static/icons/document.png" alt="with text" title="with document" />
			</xsl:when>
			<xsl:otherwise>
				<img src="/static/icons/biblio.png" alt="bibilografy" title="bibliography only" />
			</xsl:otherwise>
                </xsl:choose>
                    <xsl:element name="a">
                        <xsl:attribute name="href">
