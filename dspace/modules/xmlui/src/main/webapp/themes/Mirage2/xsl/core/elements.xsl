...
-->
	...
	<xsl:choose>
		<!-- <xsl:when test="contains(@n, 'recent-submission')">
			LAST SUBMITTED
		</xsl:when>
		 Change order of search-browse and detail-view 
		<xsl:when test="contains(@id, 'community-home')">
			<xsl:apply-templates select="*[@n='community-view']"/>
			<xsl:apply-templates select="*[@n='community-search-browse']"/>
			<xsl:apply-templates select="*[@n='community-recent-submission']"/>
		</xsl:when>
		<xsl:when test="contains(@id, 'collection-home')">
                        <xsl:apply-templates select="*[@n='collection-view']"/>
			<xsl:apply-templates select="*[@n='collection-search-browse']"/>
			<xsl:apply-templates select="*[@n='collection-recent-submission']"/>
		</xsl:when>-->
    ...
    <xsl:template name="renderHead">
		...
		<!-- <xsl:if test="contains(../@rend, 'collection') or contains(../@rend, 'community')">
			<xsl:if test="$is_first_head_on_page">
			<a class="hidden-xs hidden-sm">
				<xsl:attribute name="href">
					<xsl:value-of select="//dri:metadata[@element='feed'][2]"/>
				</xsl:attribute>
					<img class="btn-xs" src="/static/icons/feed.png"/>
			</a>
			</xsl:if>
		</xsl:if> -->
	</xsl:element>
    ...
    <xsl:template match="dri:div/dri:head" priority="3">
        <xsl:choose>
		<xsl:when test="contains(../@id, 'recent-submission')">
			<div class="row">
				<div class="col-lg-8 col-md-8">
		        <xsl:call-template name="renderHead">
		            <xsl:with-param name="class">ds-div-head recent-submission</xsl:with-param>
		    </xsl:call-template>
		    		</div>
				<div class="col-lg-4 col-md-4 rs">
		    <a>
			    <xsl:attribute name="href"><xsl:value-of select="concat('/', //dri:metadata[@element='request' and @qualifier='URI'], '/browse?rpp=20&amp;sort_by=7&amp;type=dateissued&amp;order=DESC')"/></xsl:attribute>
			    <i18n:text>xmlui.all.publications</i18n:text>
		    </a> 
		    		</div>
			</div>
    		</xsl:when>
